...
class player {
  ...
  constructor(name = "loading...",points=0,team=null){
...
socket.on('point',function(inv) {
  ...
  for (let i = 0 ; i < inv.length ; i++)
    if (inv[i]!=0)
      for (let j = 0 ; j < pl.items.length ; j++)
        if (pl.items[j]==0){
          pl.items[j]=inv[i];
          break;
        }
...
function createButton(func,style,v="",d=document.body):any {
...
trashbutton = createButtoni(function(){if (invfocus!=-1){pl.items[invfocus]=0;invfocus=-1;}},"position: absolute; left: 32px; top: 312px;width:64px; height:64px;");
}
...
var passivemode:number = 0;//0 - speed , 1 - protection , 2 - healing "for mobile only"
...
function updatefunc() {
  ...
  else if (isMobile) chatelements[0].style.fontSize = '7px';
  ...
  if (pl.hlen < 0.1){
    ...
    if (isMobile){
      ...
      for (let i = 0 ; i < ongoingTouches.length ; i += 1){
          ...
          let tt = (new vec(ongoingTouches[i].pageX-canvas.width/4,-(ongoingTouches[i].pageY-(canvas.height*4/6)))).norm();
          v = v.add(  new vec(Math.round(tt.x),Math.round(tt.y) ));

        }else if (ongoingTouches[i].pageX<canvas.width*3/4){
          keys.main=true;
        }else{
          keys.off=true;
        }
    ...
    if (v.lensq() > 0.1){
      ...
      if (v.dot(directions[1]) >  v.dot(directions[0])-0.01 && v.dot(directions[1]) >  v.dot(directions[2])-0.01 && v.dot(directions[1]) >  v.dot(directions[3])-0.01)
        pl.dir=1;
      if (isMobile){
          let vt = (new vec(-1/2,-1/3)).add(v.mul(new vec(0.2*(canvas.height/canvas.width),0.2)));
          gl.useProgram(dynshad.prog);
